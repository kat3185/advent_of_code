...
require 'pry'
class CheckSum
  class YouLiedToMe < StandardError; end
  attr_reader :spreadsheet
  def initialize(input)
    @spreadsheet = sanitize(input)
  end

  def call
    spreadsheet.inject(0) do |sum, row|
      sum + difference(row)
    end
  end
  ...
  def call_part_two
    spreadsheet.inject(0) do |sum, row|
      sum + remainderless_division(row)
    end
  end
  ...
  def remainderless_division(row)
    values = remainerless_values(row).sort
    raise YouLiedToMe unless values.count == 2
    values.last / values.first
  end

  def remainerless_values(row)
    row.find_all do |num|
      row.reject {|value| value == num }.any? do |value|
